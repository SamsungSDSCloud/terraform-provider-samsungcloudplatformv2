...
func (client *Client) GetTransitGatewayInfo(ctx context.Context, transitGatewayId string) (*vpc.TransitGatewayShowResponseV1Dot3, error) {
	req := client.sdkClient.VpcV1TransitGatewayApiAPI.ShowTransitGateway(ctx, transitGatewayId)
	resp, _, err := req.Execute()
	return resp, err
}

func (client *Client) CreateTransitGateway(ctx context.Context, dataReq TgwResource) (*vpc.TransitGatewayShowResponseV1Dot3, *http.Response, error) {
...
func (client *Client) GetTransitGatewayList(ctx context.Context, request TgwDataSource) (*vpc.TransitGatewayListResponseV1Dot2, *http.Response, error) {
	...
	if !request.FirewallConnectionState.IsNull() {
		req = req.FirewallConnectionState(vpc.TransitGatewayFirewallConnectionState(request.FirewallConnectionState.ValueString()))
	}
	if !request.UplinkZoneState.IsNull() {
		req = req.UplinkZoneState(vpc.TransitGatewayUplinkZoneState(request.UplinkZoneState.ValueString()))
	}
