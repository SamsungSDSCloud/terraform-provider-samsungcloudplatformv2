...
func extractStringList(attr types.List) []string {
	elems := attr.Elements()
...
func attrToStringList(val attr.Value) ([]string, bool) {
	...
	return extractStringList(list), true
}

func buildPrincipal(stmtAttrs map[string]attr.Value) (organization.Principal, bool) {
	// principal_str 우선
	if v, ok := stmtAttrs["principal_str"]; ok && !v.IsNull() && !v.IsUnknown() {
		s := v.(types.String).ValueString()
		return organization.Principal{String: &s}, true
	}

	v, ok := stmtAttrs["principal"]
	if !ok || v.IsNull() || v.IsUnknown() {
		return organization.Principal{}, false
	}

	principalObj, ok := v.(types.Object)
	if !ok {
		return organization.Principal{}, false
	}

	scpVal, ok := principalObj.Attributes()["scp"]
	if !ok || scpVal.IsNull() || scpVal.IsUnknown() {
		return organization.Principal{}, false
	}

	scps, ok := attrToStringList(scpVal)
	if !ok {
		return organization.Principal{}, false
	}

	scpMap := map[string][]string{"scp": scps}
	return organization.Principal{MapmapOfStringarrayOfString: &scpMap}, true
}

func buildStatement(stmtObj types.Object) (organization.DelegationPolicyStatement, bool) {
	attrs := stmtObj.Attributes()
	stmt := organization.NewDelegationPolicyStatement()

	if v, ok := attrs["sid"]; ok && !v.IsNull() && !v.IsUnknown() {
		stmt.SetSid(v.(types.String).ValueString())
	}
	if v, ok := attrs["effect"]; ok && !v.IsNull() && !v.IsUnknown() {
		stmt.SetEffect(v.(types.String).ValueString())
	}
	if actions, ok := attrToStringList(attrs["action"]); ok {
		stmt.SetAction(actions)
	}
	if resources, ok := attrToStringList(attrs["resource"]); ok {
		stmt.SetResource(resources)
	}
	if principal, ok := buildPrincipal(attrs); ok {
...
func (o DelegationPolicyDocumentValue) toSdkType() (*organization.DelegationPolicyDocument, error) {
	...
	if !o.Statement.IsNull() && !o.Statement.IsUnknown() {
		elems := o.Statement.Elements()
		statements := make([]organization.DelegationPolicyStatement, 0, len(elems))

		for _, elem := range elems {
			stmtObj, ok := elem.(types.Object)
			if !ok {
				continue
			}
			if stmt, ok := buildStatement(stmtObj); ok {
...
func (o DelegationPolicyResource) ToSdkType() (*organization.DelegationPolicyCreateRequest, error) {
	...
	if !o.Document.Statement.IsNull() && !o.Document.Statement.IsUnknown() {
		statements := make([]organization.DelegationPolicyStatement, 0)
		for _, elem := range o.Document.Statement.Elements() {
			stmtObj, ok := elem.(types.Object)
			if !ok {
				continue
			}
