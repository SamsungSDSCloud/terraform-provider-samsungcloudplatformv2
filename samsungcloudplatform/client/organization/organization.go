...
func (client *Client) GetAccount(ctx context.Context, accountId string, organizationId string, lazyPolicy *bool) (*organization.AccountShowResponse, error) {
	req := client.sdkClient.OrganizationV1AccountsAPIsAPI.ShowAccount(ctx, accountId)
	if organizationId != "" {
		req = req.OrganizationId(organizationId)
	}
	if lazyPolicy != nil {
		req = req.LazyPolicy(*lazyPolicy)
	}
	resp, _, err := req.Execute()
	return resp, err
}
...
func buildScpStatement(stmtObj types.Object) (organization.ServiceControlPolicyStatement, bool) {
	...
	if notActions, ok := attrToStringList(attrs["not_action"]); ok {
...
func (o ServiceControlPolicyDocumentValue) toSdkType() (*organization.ServiceControlPolicyDocument, error) {
	...
	if !o.Statement.IsNull() && !o.Statement.IsUnknown() {
		...
		statements := make([]organization.ServiceControlPolicyStatement, 0, len(elems))
		...
		for _, elem := range elems {
			...
			if stmt, ok := buildScpStatement(stmtObj); ok {
...
func documentObjectToSdkType(docObj types.Object) (*organization.ServiceControlPolicyDocument, error) {
	...
	if stmtAttr, ok := attrs["statement"]; ok && !stmtAttr.IsNull() && !stmtAttr.IsUnknown() {
		if statementList, ok := stmtAttr.(types.List); ok {
			elems := statementList.Elements()
			statements := make([]organization.ServiceControlPolicyStatement, 0, len(elems))
