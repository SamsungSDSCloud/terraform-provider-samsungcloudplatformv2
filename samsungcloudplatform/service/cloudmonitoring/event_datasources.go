...
var (
	_ datasource.DataSource              = &cloudMonitoringEventDataSources{}
	_ datasource.DataSourceWithConfigure = &cloudMonitoringEventDataSources{}
)

func NewCloudMonitoringEventDataSources() datasource.DataSource {
	return &cloudMonitoringEventDataSources{}
}

type cloudMonitoringEventDataSources struct {
...
func (d *cloudMonitoringEventDataSources) Metadata(_ context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_cloudmonitoring_events" // service 의 metadata 를 {{ provider명 }}_{{ 서비스명 }}_{{ 단수형 리소스명 }} 형태로 추가한다.
}

func (d *cloudMonitoringEventDataSources) Schema(ctx context.Context, _ datasource.SchemaRequest, resp *datasource.SchemaResponse) { // 아직 정의하지 않은 Schema 메서드를 추가한다.
...
func (d *cloudMonitoringEventDataSources) Configure(_ context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *cloudMonitoringEventDataSources) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) { // 아직 정의하지 않은 Read 메서드를 추가한다.
	var state cloudmonitoring.EventDataSourceIds
	...
	event, err := d.client.GetEventList(state.ResourceType, state.ProductResourceId, state.EventState, state.QueryStartDt, state.QueryEndDt)
	if err != nil {
		resp.Diagnostics.AddError(
