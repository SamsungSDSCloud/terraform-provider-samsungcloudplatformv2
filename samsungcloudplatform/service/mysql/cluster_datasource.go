...
var (
	_ datasource.DataSource              = &mysqlClusterDataSource{}
	_ datasource.DataSourceWithConfigure = &mysqlClusterDataSource{}
)

func NewMysqlClusterDataSource() datasource.DataSource {
	return &mysqlClusterDataSource{}
}

type mysqlClusterDataSource struct {
	config  *scpsdk.Configuration
	client  *mysql.Client
	clients *client.SCPClient
}

func (d *mysqlClusterDataSource) Metadata(_ context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_mysql_cluster"
}

func (d *mysqlClusterDataSource) Schema(_ context.Context, _ datasource.SchemaRequest, resp *datasource.SchemaResponse) {
					...
					common.ToSnakeCase("InitConfigOption"): schema.SingleNestedAttribute{
						...
						Attributes: map[string]schema.Attribute{
							common.ToSnakeCase("DatabaseCharacterSet"): schema.StringAttribute{
								Description: "DatabaseCharacterSet",
								Computed:    true,
							},
							common.ToSnakeCase("DatabaseCaseSensitive"): schema.BoolAttribute{
								Description: "DatabaseCaseSensitive",
								Computed:    true,
							},
...
func (d *mysqlClusterDataSource) Configure(_ context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *mysqlClusterDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var state mysql.ClusterDataSourceDetail
	...
	var InitConfigOption = mysql.InitConfigOption{
		...
		DatabaseCaseSensitive: types.BoolValue(true), //true값 고정. 추후 상세조회 api에 값 추가 예정
	}
	...
	var mysqlState = mysql.ClusterDetail{
	...
	mysqlObjectValue, _ := types.ObjectValueFrom(ctx, mysqlState.AttributeTypes(), mysqlState)
	state.ClusterDetail = mysqlObjectValue
