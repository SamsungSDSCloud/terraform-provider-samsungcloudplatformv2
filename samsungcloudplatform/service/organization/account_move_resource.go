...
func (r *accountMoveResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	for _, accountId := range targetAccountIds {
		err = waitForAccountMoveStatus(ctx, r.client, accountId, orgId, []string{}, []string{parentUnitId})
		if err != nil {
			detail := client.GetDetailFromError(err)
			resp.Diagnostics.AddError(
...
func waitForAccountMoveStatus(ctx context.Context, orgClient *organization.Client, accountId string, organizationId string, pendingStates []string, targetStates []string) error {
...
func (r *accountMoveResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	...
	for _, accountId := range targetAccountIds {
		accountResp, err := r.client.GetAccount(ctx, accountId, orgId, nil)
		if err != nil {
