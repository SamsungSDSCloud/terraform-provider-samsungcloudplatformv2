...
func (r *accountRemoveResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	alreadyFailed := make(map[string]bool)
	if result != nil {
		for _, failed := range result.FailedIds {
			alreadyFailed[failed.FailedId] = true
		}
	}

	var waitErrors []string
	for _, accountId := range accountIds {
		if alreadyFailed[accountId] {
			continue
		}
		err = waitForAccountRemoveStatus(ctx, r.client, accountId, orgId)
		if err != nil {
			detail := client.GetDetailFromError(err)
			waitErrors = append(waitErrors, fmt.Sprintf("account %s: %s (%s)", accountId, err.Error(), detail))
		}
	}
	...
	if len(waitErrors) > 0 {
		resp.Diagnostics.AddError(
			...
			"One or more accounts failed to be removed:\n"+strings.Join(waitErrors, "\n"),
		)
	}
...
func (r *accountRemoveResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	...
	for _, accountId := range accountIds {
		_, err := r.client.GetAccount(ctx, accountId, orgId, nil)
		if err != nil {
...
func waitForAccountRemoveStatus(ctx context.Context, orgClient *organization.Client, accountId string, organizationId string) error {
