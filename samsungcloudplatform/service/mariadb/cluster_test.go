...
func TestAccClusterResourceTest(t *testing.T) {
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccMysqlTemplate(),
			},
		},
	})
}

func testAccMysqlTemplate() string {
	return fmt.Sprintf(`%s %s %s %s %s`, getLocals("mariadb"), getNetwork(), getUtils(), getKeypair(), getMysql())
}
...
func getNetwork() string {
	return `
		...
		# PublicIP 가 IGW 보다 먼저(뒤에) 삭제되게 함
		  depends_on = [samsungcloudplatformv2_vpc_internet_gateway.my_igw,samsungcloudplatformv2_mariadb_cluster.my_mariadb]
		}
